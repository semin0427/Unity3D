...
public class BlockRoot : MonoBehaviour
{
    ...
    private GameObject BlockFolder = null;
    ...
    void Start()
    {
        BlockFolder = GameObject.Find("BlockFolder");
        this.main_camera =
    ...
    public bool checkConnection(BlockControl start)
    {
        ...
        for (int x = lx - 1; x > 0; x--)
        {
            ...
            if (next_block.pattern != start.pattern)
            { // 색이 다르면.
                break; // 루프를 빠져나간다.
            }
    ...
    public Block.NUMBER selectBlockNum() //수정필요
    {
        Block.NUMBER _number = Block.NUMBER.FIRST;
        // 이번 레벨의 레벨 데이터를 가져온다.
        LevelData level_data = this.level_control.getCurrentLevelData();
        float rand = Random.Range(0.0f, 1.0f); // 0.0~1.0 사이의 난수.
        float sum = 0.0f; // 출현 확률의 합계.
        ...
        _number = (Block.NUMBER)i; // i번째 색을 반환한다.
        return (_number);
    }
}